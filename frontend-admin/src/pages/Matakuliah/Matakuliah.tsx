...
export default function MatakuliahPage() {
  ...
  const {
    ...
    refetch: refetchMatakuliah,
  } = useAsyncQuery('admin:courses', matakuliahApi.getAll)

  const { data: jadwalList = [], isLoading: isLoadingJadwal } = useAsyncQuery(
    'admin:schedules',
    jadwalApi.getAll,
  )

  // Fetch Materi for Selected Course in Classroom Modal
  const selectedCourseId = selectedCourseForClass?.id ?? ''
  const { data: materiList = [], isLoading: isLoadingMateri } = useAsyncQuery(
    `admin:materials:${selectedCourseId}`,
    () => materiApi.getAllByMatakuliah(selectedCourseId),
    [selectedCourseId],
  ...
  const createMutation = useAsyncMutation({
    mutationFn: ({ data, thumbnail }: { data: typeof formData; thumbnail?: File }) =>
      matakuliahApi.create(data, thumbnail),
    onSuccess: () => {
      refetchMatakuliah()
      setIsAddModalOpen(false)
  ...
  const updateMutation = useAsyncMutation({
    mutationFn: ({
      ...
      thumbnail?: File
    }) => matakuliahApi.update(id, data, thumbnail),
    onSuccess: () => {
      refetchMatakuliah()
      setIsAddModalOpen(false)
  ...
  const { heroCourse, heroSchedule, heroStatus, heroTimeLabel } = useMemo(() => {
    ...
    const upcomingToday = jadwalList
      .filter(
        (j) => j.hari.toLowerCase() === currentDayName.toLowerCase() && j.jamMulai > currentTimeStr,
      )
    ...
    <div className={styles.container}>
      ...
      <header className={styles.header}>
        ...
        <div className={styles.headerActions}>
          ...
          <button type="button" className={styles.addBtn} onClick={handleOpenAddModal}>
            <Plus size={18} />
        ...
        <section className={styles.heroCard}>
          <div className={styles.heroLeft}>
            <div className={styles.heroBadgeRow}>
              ...
              <span className={styles.heroCourseCode}>{heroCourse.kode} • Minggu 14</span>
            </div>
      ...
      <div className={styles.filterBar}>
        ...
        <span className={styles.classCountText}>{filteredCourses.length} kelas ditampilkan</span>
      </div>
      ...
      <div className={styles.grid}>
        {filteredCourses.map((course, idx) => {
          ...
          const isCompleted =
            filterTab === 'selesai' || (idx === filteredCourses.length - 1 && idx > 2)
          ...
          return (
            ...
            >
              ...
              <div className={styles.cardFooter}>
                ...
                <span className={styles.scheduleLink}>{schedInfo.label}</span>
              </div>
            </div>
          )
        })}
            ...
            >
              <div className={styles.modalHeader}>
                <div className={styles.modalTitleArea}>
                  ...
                  <h2 id="classroom-modal-title" className={styles.modalTitle}>
                    {selectedCourseForClass.nama}
                  </h2>
                  <div className={styles.modalMetaRow}>
                    <span>
                      Kode: <strong>{selectedCourseForClass.kode}</strong>
                    </span>
                    <span>•</span>
                    <span>
                      Jadwal:{' '}
                      <strong>
                        {getCourseScheduleInfo(selectedCourseForClass.id)
                          .label.replace('→', '')
                          .trim()}
                      </strong>
                    </span>
                    <span>•</span>
                    <span>
                      Total: <strong>{materiList.length} Materi</strong>
                    </span>
                  </div>
                </div>
                <button
                ...
                {isLoadingMateri ? (
                  ...
                  <div className={styles.emptyMateri}>
                    ...
                    <p style={{ margin: 0, fontSize: '0.85rem' }}>
                      Mata kuliah ini belum memiliki materi terbit. Tambahkan materi pertama Anda
                      sekarang.
                    </p>
                    <button
                      type="button"
          ...
          <div className={styles.modalBackdrop} onClick={() => setIsAddModalOpen(false)}>
            ...
            >
              ...
              >
                ...
                <div className={styles.formGroup}>
                  <label className={styles.formLabel}>Thumbnail Mata Kuliah (Opsional)</label>
                  <label className={styles.courseThumbnailPicker}>
                    {thumbnailPreview ? (
                      <img src={thumbnailPreview} alt="Pratinjau thumbnail mata kuliah" />
                    ) : (
                      <span className={styles.courseThumbnailFallback}>
                        <ImagePlus size={26} />
                        Tambahkan gambar sampul
                      </span>
                    )}
                    <span className={styles.courseThumbnailHint}>
                    ...
                    <input
                      ...
                      onChange={(event) => {
                        ...
                        if (file.size > 5 * 1024 * 1024) {
                          setFormError('Ukuran thumbnail maksimum 5 MB.')
                          return
                        }
                        setFormError('')
