...
export const useScheduleManagement = () => {
  ...
  const saveMutation = useMutation({
    mutationFn: () => editingSchedule
      ? jadwalApi.update(editingSchedule.id, form)
      : jadwalApi.create(form),
    onSuccess: () => {
  ...
  const openCreateModal = useCallback((day: HariKuliah = 'Senin') => {
    setEditingSchedule(null);
    setForm({ ...INITIAL_SCHEDULE_FORM, hari: day, matakuliahId: courses[0]?.id ?? '' });
    setFormError('');
    setIsModalOpen(true);
  }, [courses]);
  ...
  const removeSchedule = useCallback((schedule: Jadwal) => {
