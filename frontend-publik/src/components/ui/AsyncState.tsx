...
export function LoadingCards() {
  return <div className="card-grid">{[1, 2, 3].map((item) => <div className="skeleton-card" key={item}><i /><span /><span /></div>)}</div>
}

export function QueryState({ error, empty, children }: { error?: unknown; empty?: boolean; children: ReactNode }) {
