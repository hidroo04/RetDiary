...
export function LearningHero({
  ...
  const style = image
    ? ({ '--learning-hero-image': `url("${image}")` } as CSSProperties)
    : undefined
  return (
    <section className={`learning-hero ${image ? 'has-thumbnail' : 'fallback-hero'}`} style={style}>
      ...
      <div className="learning-hero-content">
        {backTo && (
          <Link to={backTo} className="hero-back-button">
            <span>←</span>
            {backLabel}
          </Link>
        )}
